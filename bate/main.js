...
function create()
{
	...
	platform = game.add.tileSprite(0, 400-32, 800, 32, 'game', 1);
	...
	player = game.add.sprite( 50, 400-31, 'player');
	ball = game.add.sprite(530, 340, 'ball');

	gameObjects.add(player);
	gameObjects.add(ball);

	game.physics.arcade.enable([player, ball, platform]);

	player.body.collideWorldBounds = true;
	player.animations.add('death', [0,1,2,3], 5, false);
    player.animations.add('hit', [4,5,6,5,4], 10, false).onComplete.add(onHitComplete, this);;
    player.animations.add('idle', [7], 1, false);
    player.animations.play('idle');
    player.anchor.setTo(0.5,1);
	player.scale.setTo(2);
    player.body.setSize(10, 30, 0, 0);	
	
	ball.animations.add('show', [0,1,2,3], 12, true);	
	ball.animations.play('show');
	ball.scale.setTo(2);
...
{	
	...
	game.physics.arcade.collide(ball, player, onPlayerHit, null, this);
}
